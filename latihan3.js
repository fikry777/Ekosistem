...
selanjutnya.addEventListener('click', function () {
    ...
    if (sekolahnya.value == "0") {
        if (sekolahnya.className.indexOf('tt_salah') == -1) {
            sekolahnya.className += ' tt_salah';
        }
    } else {
        sekolahnya.className = sekolahnya.className.replace('tt_salah', '');
        if (sekolahnya.value == "1") {
            sekolahfix = "SDN Telawang 1";
        } else if (sekolahnya.value == "2") {
            sekolahfix = "SDN Sungai 1";
        }      
        cek += 1;
        cek3 = 3;
    }
    ...
    if (dat.readyState == 4 && dat.status == 200) {
        ...
        var xx =setInterval(function(){
            if(nilaiwktu ==1){
                ...
                createTask(sekolahfix, namanya.value.toUpperCase(), kelasfix, hasilakhir, waktunya, harinya, jawabannya);
